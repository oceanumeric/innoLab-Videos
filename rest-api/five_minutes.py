...
class Ch1(MovingCameraScene):
    def construct(self):
        ...
        # add { REST }
        rest_text1 = Text("{", font="Open Sans", color="#37CB8F", weight=BOLD).shift(
            UP * 0.2 + LEFT * 1.7
        )
        ...
        rest_text3 = Text("}", font="Open Sans", color="#37CB8F", weight=BOLD)
        ...
        api_abbr = Text("API", font="Roboto", color=theme_color, weight=BOLD).shift(
            LEFT * 7 + UP * 0.5
        )
        ...
        def _abb_text(text1, text2, shift_x, shift_y):
            abbr = Text(text1, font="Roboto", color=theme_color, weight=BOLD).shift(
                LEFT * shift_x + UP * shift_y
            )
        ...
        # pc image
        pc_icon = (
            BootstrapSVGMobject("pc-display-horizontal", color=theme_color_blue)
            .scale(0.8)
        ...
        # sever image
        sever_image = (
            SVGMobject("../assets/server.svg").scale(0.8).shift(RIGHT * 4 + DOWN * 0.2)
        )
        ...
        # add text
        request_text = (
            Text("Request", font="Roboto", color=theme_color_blue, weight=BOLD)
            .scale(0.4)
            .shift(UP * 0.2)
        )
        ...
        # add text
        response_text = (
            Text("Response", font="Roboto", color=theme_color_blue, weight=BOLD)
            .scale(0.4)
            .shift(DOWN * 0.2)
        )
        ...
        # add text
        cloud_text = (
            Text("{...}", font="Roboto", color=theme_color_gray, weight=BOLD)
            .scale(0.4)
            .shift(DOWN * 0.1)
        )
        rest_api_text = (
            Text("REST API", font="Roboto", color=theme_color_gray)
            .scale(0.4)
            .next_to(cloud_text, DOWN, buff=0.1)
        )
        ...
        rest_exp = _abb_text(
            "REST", "stands for REpresentational State Transfer", 6.2, 1
        )
        ...
        not_specification = Text(
            "Not a specification", font="Roboto", color=theme_color_gray
        )
        not_specification.next_to(document_svg, DOWN, buff=0.2)
        not_specification.scale(0.7)

        self.play(FadeIn(document_svg), Write(not_specification), run_time=0.7)

        self.play(FadeOut(document_svg), FadeOut(not_specification))

        # rectangle
        rest_api_rules = Text("REST API Rules", font="Roboto", weight=BOLD).scale(0.5)
        ...
        def _text_box(text):
            text = Text(text, font="Roboto", color=theme_color_blue, weight=BOLD)
